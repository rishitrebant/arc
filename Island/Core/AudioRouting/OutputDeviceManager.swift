...
final class AudioOutputDeviceManager: ObservableObject {
    ...
    private static func readDefaultOutputDevice()
        -> AudioDeviceID
    {
    ...
    ) -> [OutputDevice]
    {
        ...
        return
            deviceIDs.compactMap {
                deviceID in

                // Must actually have audio output channels.
                guard
                    Self.hasOutputChannels(
                        deviceID
                    )
                else {
                    return nil
                ...
                // Only keep physical / currently-connected
                // audio transports. This filters out virtual,
                // aggregate and software-created devices.
                guard
                    Self.isPhysicalOutputDevice(
                        deviceID
                    )
                else {
                    return nil
